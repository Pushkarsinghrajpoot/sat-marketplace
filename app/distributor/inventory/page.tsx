...
import {
  ...
  ShoppingCart, DollarSign, Boxes, Save, Plus,
...
const DECREMENTS = new Set(['SALE', 'DAMAGE', 'TRANSFER']);
...
export default function DistributorInventoryPage() {
    ...
    try {
      const res = await fetch('/api/inventory/adjust', {
        ...
        body: JSON.stringify({
          ...
          changeType: type,
          quantityChange: DECREMENTS.has(type) ? -Math.abs(delta) : Math.abs(delta),
          notes: notes || null,
          createdBy: user?.id,
        }),
      });
  ...
  const handleAdjust = async (productId: string) => {
    setSubmitting(true);
    await runAdjust(productId, adjustQty, adjustType, adjustNotes, () => {
      setAdjustingId(null); setAdjustQty(''); setAdjustNotes('');
    });
    setSubmitting(false);
  };
  ...
  const handleBulkAdjust = async () => {
    ...
    for (const pid of selected) {
      await runAdjust(pid, bulkQty, bulkType, bulkNotes, () => { ok++; });
    }
  ...
  // ─── Update threshold / reorder qty inline ───────────────
  const handleThresholdSave = async (productId: string, threshold: number, reorderQty: number) => {
    const { error } = await supabase
      .from('products')
      .update({ low_stock_threshold: threshold, reorder_quantity: reorderQty })
      .eq('id', productId);
  ...
  const handleExport = () => {
    ...
    const csv = [
      ...
      ...products.map(p => [
        p.sku, `"${p.name}"`, p.brand || '', p.price, p.cost_price || 0,
        p.inventory ?? 0, p.low_stock_threshold ?? 0, p.reorder_quantity ?? 0,
        p.warehouse_location || '', ((p.price ?? 0) * (p.inventory ?? 0)).toFixed(2)
      ].join(','))
    ].join('\n');
  ...
  const topByValue = useMemo(() =>
    [...products].sort((a, b) => (b.price * b.inventory) - (a.price * a.inventory)).slice(0, 5),
    [products]
  );
  ...
  const toggleAll = () => {
    ...
    else setSelected(new Set(stockRows.map(p => p.id)));
  };
        ...
        {[
          ...
          <Card key={label}>
            <CardContent className="p-4">
              ...
              <p className={`text-lg font-bold text-${color === 'orange' ? 'orange' : color === 'red' ? 'red' : 'gray'}-${color === 'orange' ? '600' : color === 'red' ? '600' : '900'}`}>{value}</p>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      {reorderNeeded.length > 0 && (
        <div className="mb-5 flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
          ...
          <p className="text-sm text-amber-800 font-medium">
            <strong>{reorderNeeded.length}</strong> product{reorderNeeded.length !== 1 ? 's need' : ' needs'} restocking
          </p>
          <button onClick={() => setActiveTab('reorder')} className="ml-auto text-xs font-semibold text-amber-700 underline underline-offset-2">
            View Reorder Queue →
          </button>
        </div>
      )}
        ...
        {TABS.map(({ key, label, icon: Icon }) => (
          ...
          >
            ...
            {key === 'reorder' && reorderNeeded.length > 0 && (
              <span className={`text-xs px-1.5 py-0.5 rounded-full font-semibold ${activeTab === key ? 'bg-red-500 text-white' : 'bg-red-100 text-red-600'}`}>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="space-y-2">
                {[
                  { label: 'In Stock', count: inStock, color: 'bg-green-400' },
                  { label: 'Low Stock', count: lowStock, color: 'bg-orange-400' },
          ...
          <Card className="lg:col-span-3">
            ...
            <CardContent className="p-0">
              {logs.length === 0 ? (
                <p className="text-sm text-gray-400 text-center py-8">No movements recorded yet. Adjust stock to get started.</p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-gray-50 border-b border-gray-100">
                      <tr>
                        {['Product', 'Type', 'Change', 'Stock After', 'Date'].map(h => (
                      ...
                      {logs.slice(0, 8).map(log => (
                        <tr key={log.id} className="hover:bg-gray-50">
                          <td className="px-4 py-2.5">
                            <p className="text-sm font-medium text-gray-800">{log.products?.name}</p>
                            <p className="text-xs text-gray-400 font-mono">{log.products?.sku}</p>
                          </td>
                          <td className="px-4 py-2.5">{logTypePill(log.change_type)}</td>
                          <td className="px-4 py-2.5">
                            <span className={`text-sm font-bold flex items-center gap-1 ${log.quantity_change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                              {log.quantity_change >= 0 ? <ArrowUpCircle className="h-3.5 w-3.5" /> : <ArrowDownCircle className="h-3.5 w-3.5" />}
            ...
            {selected.size > 0 && (
              <div className="flex items-center gap-2 bg-indigo-50 border border-indigo-200 rounded-xl px-3 py-2">
                ...
                <select value={bulkType} onChange={e => setBulkType(e.target.value)}
                  className="h-8 text-xs border border-gray-200 rounded-lg px-2 bg-white focus:outline-none">
                ...
                <Button size="sm" className="h-8 text-xs" disabled={bulkSubmitting} onClick={handleBulkAdjust}>
                  {bulkSubmitting ? 'Applying...' : 'Apply Bulk'}
                </Button>
                  ...
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={8} className="px-4 py-16 text-center">
                        <Boxes className="h-10 w-10 text-gray-200 mx-auto mb-3" />
                      ...
                      <>
                        <tr key={p.id} className={`hover:bg-gray-50 ${selected.has(p.id) ? 'bg-indigo-50' : ''}`}>
                          ...
                          <td className="px-4 py-3">
                            ...
                            <p className="text-xs font-mono text-gray-400">{p.sku} · {p.brand}</p>
                          </td>
                          <td className="px-4 py-3 min-w-[140px]">
                            <StockBar value={p.inventory ?? 0} max={maxStock} />
                          </td>
                          ...
                          <td className="px-4 py-3 text-right">
                            <Button variant="outline" size="sm" className="h-8 text-xs"
                              onClick={() => { setAdjustingId(isAdj ? null : p.id); setAdjustQty(''); setAdjustType('RESTOCK'); setAdjustNotes(''); }}>
                          ...
                          <tr key={`adj-${p.id}`}>
                            <td colSpan={8} className="px-4 py-4 bg-indigo-50 border-l-4 border-indigo-500">
                              <div className="flex flex-wrap gap-3 items-end">
                                <div>
                                  ...
                                  <select value={adjustType} onChange={e => setAdjustType(e.target.value)}
                                    className="h-9 text-sm border border-gray-200 rounded-lg px-2 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-400">
                                    {CHANGE_TYPES.map(ct => <option key={ct.value} value={ct.value}>{ct.label}</option>)}
                                  </select>
                                </div>
                                <div>
                              ...
                              <p className="mt-2 text-xs text-gray-500">
                                Current: <strong>{p.inventory ?? 0} units</strong> · Sale/Damage/Transfer will subtract; Restock/Return/Adjustment will add.
                              </p>
                            </td>
                          </tr>
                        )}
      ...
      {activeTab === 'reorder' && (
        <div className="space-y-4">
          {reorderNeeded.length === 0 ? (
            ...
            <>
              <p className="text-sm text-gray-500">{reorderNeeded.length} product{reorderNeeded.length !== 1 ? 's are' : ' is'} at or below the reorder point. Restock to prevent stockouts.</p>
              {reorderNeeded.map(p => (
                <ReorderRow key={p.id} product={p} orgId={orgId!} userId={user?.id!}
                  onDone={() => { fetchProducts(); fetchLogs(); }} />
              ))}
            </>
          )}
        </div>
        ...
        <>
          <div className="flex flex-wrap gap-3 mb-4">
            ...
            <select value={logTypeFilter} onChange={e => setLogTypeFilter(e.target.value)}
              className="h-10 text-sm border border-gray-200 rounded-lg px-3 bg-white focus:outline-none">
          ...
          <Card>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-100">
                  <tr>
                    {['Product', 'SKU', 'Type', 'Before', 'Change', 'After', 'Notes', 'By', 'Date'].map(h => (
...
// ── Reorder row sub-component ────────────────────────────────────────────────
function ReorderRow({ product: p, orgId, userId, onDone }: { product: any; orgId: string; userId: string; onDone: () => void }) {
