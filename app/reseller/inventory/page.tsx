...
import {
  ...
  Search, Plus, RotateCcw, Download, History, Edit,
...
function stockBadge(inventory: number, threshold: number) {
  if (inventory === 0)
  ...
  const handleExport = () => {
    ...
    const csv = [
      ...
      ...products.map(p => [
        ...
        p.inventory ?? 0, p.low_stock_threshold ?? 0,
        (p.inventory ?? 0) === 0 ? 'OUT_OF_STOCK' : (p.inventory ?? 0) <= (p.low_stock_threshold ?? 10) ? 'LOW_STOCK' : 'IN_STOCK'
      ].join(','))
    ].join('\n');
    const blob = new Blob([csv], { type: 'text/csv' });
    const a = Object.assign(document.createElement('a'), {
      href: URL.createObjectURL(blob),
      download: `my-inventory-${new Date().toISOString().split('T')[0]}.csv`
    });
    a.click();
    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-[14px] text-gray-500">Track stock levels for your marketplace products</p>
        </div>
        ...
        <Card>
          <CardContent className="p-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-gray-500 font-medium">My Products</p>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50 border-b border-gray-100">
                    <tr>
                      {['Product', 'Type', 'Before', 'Change', 'After', 'Notes', 'Date'].map(h => (
                    ...
                    <tr>
                      <td colSpan={7} className="px-5 py-16 text-center">
                        ...
                        <p className="text-gray-400 text-sm">
                          {activeTab === 'low' ? 'No low-stock products' :
                           activeTab === 'out' ? 'All products are in stock 🎉' :
                      ...
                      <>
                        <tr key={product.id} className="hover:bg-gray-50">
                          <td className="px-5 py-4">
                            <div className="flex items-center gap-3">
                              <div className="w-9 h-9 bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-lg flex items-center justify-center flex-shrink-0">
                                <span className="text-xs font-bold text-indigo-400">{(product.brand || product.name || 'P').charAt(0)}</span>
                              </div>
                              <div>
                          ...
                          <td className="px-5 py-4">
                            {stockBadge(product.inventory ?? 0, product.low_stock_threshold ?? 10)}
                          </td>
                          <td className="px-5 py-4 text-right">
                            <div className="flex items-center justify-end gap-1">
                              ...
                              >
                                <RotateCcw className="h-3 w-3 mr-1" />Adjust
                              </Button>
                          ...
                          <tr key={`adj-${product.id}`}>
                            <td colSpan={7} className="px-5 py-4 bg-blue-50 border-l-4 border-blue-400">
                              <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-end">
                                ...
                                <div className="flex gap-2">
                                  <Button size="sm" className="h-9" disabled={submitting} onClick={() => handleAdjust(product)}>
                                    {submitting ? 'Saving...' : 'Save'}
                                  </Button>
                                  <Button variant="ghost" size="sm" className="h-9" onClick={() => setAdjustingId(null)}>
                                    Cancel
                                  </Button>
                                </div>
                              </div>
                              <p className="text-xs text-gray-500 mt-2">
                                Current stock: <strong>{product.inventory ?? 0}</strong> units. Sale/Damage subtracts; all others add.
                              </p>
                            </td>
                          </tr>
                        )}
